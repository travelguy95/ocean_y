...
def load_darcy_pt(data_path, 
    ...
    """
    data = torch.load(Path(data_path).joinpath(f'ocean_M_train_{train_resolution}_advanced.pt').as_posix())
    ...
    data = torch.load(Path(data_path).joinpath(f'ocean_M_test_{train_resolution}_advanced.pt').as_posix())
    #####################################
    ...
    # n_test = n_tests.pop(idx)
    # test_batch_size = test_batch_sizes.pop(idx)
    # data = torch.load(Path(data_path).joinpath(f'ocean_M_test_{1024}.pt').as_posix())
    #####################################
